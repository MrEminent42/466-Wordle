...
class WordleGame:
    ...
    def guess(self, guess) -> list[Color]:
        tiles = []
        if len(guess) != 5:
            raise ValueError(
        ...
        if self.board.get_num_rows() >= 6:
            ...
            print("WIN!")
            self._is_complete = self.win = True
    ...
    def get_colors(self, guess: str) -> list[Color]:
        ...
        occurrences_remaining = Counter(self.answer)
        ...
        for i, char in enumerate(guess):
            ...
            if colors[i] == Color.GREEN or occurrences_remaining[char] == 0:
                continue
